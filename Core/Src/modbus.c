...
static unsigned char table_crc_hi[] = { 0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0,
...
static unsigned char table_crc_lo[] = { 0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03,
...
static uint8_t modbusCRC[2] = { 0 };
uint8_t modbusMsgData[20] = { 0 };
...
int8_t handleComms(UART_HandleTypeDef *huart, uint16_t *registers)
{
	...
	HAL_UART_Receive(huart, modbusMsgData, 8, 20);

	//check address - address 0x02 is hardcoded
	if (modbusMsgData[0] != 0x02)
	{
	...
	//check crc
	crc16(modbusMsgData, 8);
	if (modbusCRC[0] != modbusMsgData[6] | modbusCRC[1] != modbusMsgData[7])
	{
		//crc was bad
		return MSG_CORRUPT;
	}
